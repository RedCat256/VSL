...
(define parser%
  (class object%
    ...
    (define/private (if-stmt)
      (let-values ([(tok test if-arm then-arm)
                    (values prev #f #f #f)])
        (consume '|(| "Expect '(' after if.")
        (set! test (expr))
        (consume '|)| "Expect ')' after if test.")
        (set! if-arm (stmt))
        (when (_match 'else)
          (set! then-arm (stmt)))
        (stmt:if tok test if-arm then-arm)))

    (define/private (while-stmt)
      (let-values ([(tok test body) (values prev #f #f)])
        (incf loop-depth)
        (consume '|(| "Expect '(' after while.")
        (set! test (expr))
        (consume '|)| "Expect ')' after while test.")
        (set! body (stmt))
        (incf loop-depth -1)
        (stmt:while tok test body)))

    (define/private (for-stmt)
      (let-values ([(tok init test increment body)
                    (values prev #f #f #f #f)])
        (incf loop-depth)
        ...
        (unless (check '|;|)
          (set! test (expr)))
        (consume '|;| "Expect ';' after loop test.")
        (unless (check '|)|)
          (set! increment (expr)))
        (consume '|)| "Expect ')' after for clauses.")
        (set! body (stmt))
        (incf loop-depth -1)
        (stmt:for tok init test increment body)))
